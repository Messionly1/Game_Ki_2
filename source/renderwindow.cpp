...
void RenderWindow::renderMusicToggle(Anything& thing, SDL_Texture* onTexture, SDL_Texture* offTexture, bool isMusicOn) {
    SDL_Rect src;
    src.x = thing.getCurrentFrame().x;
    src.y = thing.getCurrentFrame().y;
    src.w = thing.getCurrentFrame().w;
    ...
    dst.w = src.w;
    dst.h = src.h;
    SDL_RenderCopy(renderer, isMusicOn ? onTexture : offTexture, &src, &dst);
}
